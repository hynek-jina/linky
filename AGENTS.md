...
- Unauthenticated restore now opens a dedicated `I'm returning` onboarding step with manual/paste SLIP-39 entry, inline separator normalization, live word validation, clipboard-triggered auto-submit when a full valid share is pasted, and a password-manager/keychain load action when browser credential access is available
...
- On supported browser/PWA environments (currently at least macOS browsers), the wallet banner slot offers saving the current 20-word SLIP-39 share to the password manager/keychain until the user either saves or copies it; the banner only occupies that slot while the normal wallet early-warning banner does not apply
