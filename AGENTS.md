...
- `local-stack` — `tests/proxy-payment.spec.ts`, `tests/linkshu-migration.spec.ts`, `tests/chat-recovery.spec.ts`, `tests/chat-payment-request.spec.ts`, `tests/evolu-sync.spec.ts`, `tests/cashu-sync.spec.ts`, and `tests/password-manager-save.spec.ts`, against the docker stack with the app served as a **production build** on :5176. It declares no `webServer`; compose owns the app, so bring the stack up first.
