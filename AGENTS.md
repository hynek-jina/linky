...
- Contacts and cashu owner lanes auto-rotate independently on per-scope byte-budget thresholds with reserve (currently contacts `220`, cashu `170`), migrate their own valid rows forward, and enforce 1-minute per-type cooldown; cashu prune keeps the current contacts-aligned legacy mirror lane intact for backward compatibility
