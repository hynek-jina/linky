...
- Unauthenticated restore now opens a dedicated `I'm returning` onboarding step with manual/paste SLIP-39 entry, inline separator normalization, live word validation, clipboard-triggered auto-submit when a full valid share is pasted, an automatic password-manager/keychain prefill attempt when browser credential access is available, and a temporary explicit fallback button that re-triggers the browser prompt when needed; Safari onboarding now routes through an explicit visible save-keys step with a real username/password form before entering the app, while the restore UI keeps only the visible SLIP-39 field and uses hidden bridge inputs for browser autofill/password-manager integration
...
- Confirming that new-account profile now also best-effort invokes the browser password-manager/keychain save for the generated 20-word SLIP-39 share before reloading into the authenticated app; the wallet no longer shows a separate save-keys banner afterward
