...
- Transactions owner lane auto-rotates at the same write threshold with pointer-only switch (no transaction copy), while UI reads active + immediate previous transactions owner
...
- App shell structure lives under `apps/web-app/src/app/`:
  ...
  - `hooks/useEvoluContactsOwnerRotation.ts` owns deterministic contacts/messages/transactions owner derivation, manual contacts rotation (copy-forward), manual messages + transactions rotation (pointer-only), and owner pointer persistence
