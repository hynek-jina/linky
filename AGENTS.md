...
- `apps/web-app` Vite dev server uses HTTPS by default (`VITE_HTTPS=0` disables it only when explicitly needed) via `vite-plugin-mkcert`, so `localhost` / `127.0.0.1` get a locally trusted dev certificate; Playwright E2E also targets the HTTPS dev origin and ignores local cert issues if the CA is not trusted yet
